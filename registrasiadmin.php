...
if (isset($_POST['register_admin'])) {
    ...
    if (empty($username) || empty($nama) || empty($email) || empty($password) || empty($confirm_password)) {
    ...
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) { // Tambahkan validasi email
        $pesan_status = "<span style='color:red;'>Format email tidak valid!</span>";
    } else {
        ...
        if ($stmt_check) {
            ...
            if ($count > 0) {
                ...
                if ($stmt_insert) {
                    ...
                    if (mysqli_stmt_execute($stmt_insert)) {
                        ...
                        // Opsional: Redirect langsung setelah berhasil registrasi
                        // header("location: index.php");
                        // exit;
                    } else {
                        $pesan_status = "<span style='color:red;'>Error saat eksekusi: " . mysqli_error($conn) . "</span>";
                    }
            ...
            $pesan_status = "<span style='color:red;'>Error saat menyiapkan statement pengecekan: " . mysqli_error($conn) . "</span>";
        }
    }
}
?>
                ...
                <form method="post" action="">
                    <div class="row">
                        <div class="input-field col s12">
                            ...
                            <input id="username" type="text" class="validate" name="username" required value="<?php echo isset($_POST['username']) ? htmlspecialchars($_POST['username']) : ''; ?>">
                            <label for="username">USERNAME</label>
                        </div>
                        <div class="input-field col s12">
                            <i class="mdi mdi-account prefix"></i>
                            <input id="nama" type="text" class="validate" name="nama" required value="<?php echo isset($_POST['nama']) ? htmlspecialchars($_POST['nama']) : ''; ?>">
                            <label for="nama">NAMA LENGKAP</label>
                        </div>
                        <div class="input-field col s12">
                            <i class="mdi mdi-email-outline prefix"></i>
                            <input id="email" type="email" class="validate" name="email" required value="<?php echo isset($_POST['email']) ? htmlspecialchars($_POST['email']) : ''; ?>">
