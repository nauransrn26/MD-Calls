...
if (isset($_GET['id_jadwal'])) {
    $id_jadwal_to_delete = $_GET['id_jadwal']; // Gunakan nama variabel yang jelas
    ...
    if ($conn) {
        ...
        $query = mysqli_query($conn, "DELETE FROM jadwal_dokter WHERE id_jadwal='$id_jadwal_to_delete'");

        if ($query) {
            $_SESSION['pesan'] = "<div class='alert alert-success' style='margin-top:5px;'>
                                    <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                                    Berhasil Hapus Jadwal Dokter!
                                  </div>";
            ...
            $_SESSION['pesan'] = "<div class='alert alert-danger' style='margin-top:5px;'>
                                    <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                                    Gagal Hapus Jadwal Dokter! Error: " . mysqli_error($conn) . "
                                  </div>";
            echo "<script>window.location = '../dashboard/index.php?page=4'</script>";
            exit(); // Penting untuk menghentikan eksekusi script setelah redirect
        }
    ...
    // Jika parameter id_jadwal tidak ditemukan
